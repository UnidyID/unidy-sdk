...
import {
	onChange as onRegistrationChange,
	registrationState,
	registrationStore,
} from "../../../registration/store/registration-store";
...
export class RawField extends UnidyComponent() {
	...
	@Prop() validationFunc?: (value: string | string[]) => {
		valid: boolean;
		message?: string;
	};

	/** Emitted when the user presses Enter (or Cmd/Ctrl+Enter in textareas) to submit the field value. */
	@Event({ bubbles: true, composed: true }) uFieldSubmit!: EventEmitter<{
		field: string;
	}>;
	...
	private readProfileStore(fieldName: string): string | undefined | string[] {
		...
		if (field.radio_options) {
			const checkedOption = field.radio_options.find(
				(option: RadioOption) => option.checked,
			);
	...
	private writeProfileStore(fieldName: string, value: string | string[]) {
		...
		const key = isCustomAttribute
			? fieldName.replace("custom_attributes.", "")
			: fieldName;
	...
	private setErrors(errors: Record<string, string>) {
		switch (this.context) {
			...
			case "registration": {
				...
				const currentErrors = registrationState.errors as Record<
					string,
					string
				>;
	...
	private runExternalValidator(value: string | string[]) {
		if (this.validationFunc) {
			try {
				...
				this.logger.error(
					"External validator (validationFunc) threw an error:",
					e,
				);
				return null;
			}
	...
	private validateValue(value: string | string[]): {
		...
		if (this.pattern && typeof value === "string") {
			...
			if (!regex.test(value)) {
				return {
					valid: false,
					message: this.patternErrorMessage || "invalid_format",
				};
			}
		}
		...
		if (externalResult && !externalResult.valid) {
			return {
				valid: false,
				message: externalResult.message || "invalid_input",
			};
		}
		...
		if (this.type === "tel") {
			...
			if (
				...
				return {
					valid: false,
					message: this.invalidPhoneMessage || "invalid_phone",
				};
			}
		}
		...
		if (
			this.context === "registration" &&
			this.field === "password_confirmation" &&
			typeof value === "string" &&
			value !== ""
		) {
	...
	private onMultiToggle = (optValue: string, checked: boolean) => {
		...
		if (checked) {
			updatedValues = currentValues.includes(optValue)
				? currentValues
	...
	private onInputField = (newValue: string) => {
		...
		const storeValue =
			this.type === "tel" ? newValue.replace(/\s/g, "") : newValue;
	...
	render() {
		if (this.type === "radio") {
			...
			if (typeof this.value === "string") {
				...
				const isChecked =
					currentValue != null && String(currentValue) === this.value;
				return (
					<RadioGroup
						name={this.field}
						value={this.value}
		...
		if (this.type === "checkbox") {
			if (
				Array.isArray(this.multiSelectOptions) &&
				this.multiSelectOptions.length
			) {
