...
- **`Auth.getInstance()` returns before `initialize()` finishes**: `Auth.instance` is assigned at the top of `initialize()`, and `getInstance()` checks `Auth.instance` before `initializationPromise` — so any caller landing after config resolves but before init's awaits settle gets a not-yet-initialized instance. Token refresh is single-flight (UD-3406) so refresh races are harmless, but other init steps (redirect handling, step recovery) may still be in flight; await `auth.ready` for one-shot state reads
- **Refresh tokens rotate server-side**: each `POST /refresh_token` invalidates the token it was called with. Never issue concurrent refreshes with the same token (one succeeds, the rest 401 permanently) — all refresh paths must go through `AuthHelpers.refreshToken()`, which dedupes in-flight calls and ignores stale permanent failures when localStorage already holds a newer rotated token (another SDK module copy or tab)
- **Newsletter field errors**: `<u-error-message for="X">` inside `u-newsletter-root` reads `newsletterStore.state.errors[X]` (translated via `newsletter.errors.<identifier>`); `u-email-field` clears the whole `errors` map on every keystroke. `u-newsletter-root.submit()` sets `email_required` / `consent_required` there itself — the subscribe `u-submit-button` is disabled while the email is empty, so Enter inside `u-email-field` is the only UI path that submits an empty email (UD-3082)
...
- Backend-independent specs can skip the real-login setup project with `--no-deps`, and stub responses with `page.route` (see `brand-switcher.spec.ts`). Useful when a feature depends on backend config (multi-brand setup) that is awkward to seed
- Editing a demo `src/**/index.html` does NOT affect a static-server e2e run until you rebuild — `E2E_SDK_BASE_URL`/`CI` serve `www/`, and `bun run build` is what copies the HTML there. Re-run `bun run build && bun run e2e:configure` after touching demo markup, or tests silently exercise the old page
